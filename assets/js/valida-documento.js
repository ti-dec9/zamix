...
function validaFormato(element) {
    ...
    var resultValidationCpfCnpj = document.getElementById("resultValidationCpfCnpj");

    if (RegExp.test(strDocument) == true) {
        if (sizeStrDocument == 11) {
            if (!validaCPF(strDocument)) {
                resultValidationCpfCnpj.innerText = "Este não é um CPF válido!";
                element.select();
                return false;
            } else {
        ...
        if (sizeStrDocument == 14) {
            if (!validaCNPJ(strDocument)) {
                resultValidationCpfCnpj.innerText = "Este não é um CNPJ válido!";
                element.select();
                return false;
            } else {
        ...
        strDocument == "" ? resultValidationCpfCnpj.innerText = "" : resultValidationCpfCnpj.innerText = "'" + strDocument + "' Não tem o formato de um CPF ou um CNPJ válidos!";
        return false;
    }
}
